...
import { assertMarketResearchOutput,type MarketDailyBrief } from "@/lib/finance/market-research";
...
export default async function TodayMarketPage({ searchParams }: { searchParams: Promise<{ refresh?: string;research?:string;researchError?:string }> }) {
  ...
  let researchReport:MarketDailyBrief|null=null;if(latestArtifact){try{assertMarketResearchOutput(latestArtifact.content,researchState.context);researchReport=latestArtifact.content;}catch{researchReport=null;}}
  ...
  return <main className="mx-auto max-w-3xl px-4 py-6 pb-28"><div className="flex items-end justify-between gap-4"><div><p className="text-sm font-semibold text-primary">MARKET TODAY</p><h1 className="mt-2 text-3xl font-semibold">今日市场</h1></div><form action={refreshMarketAction}><button className="min-h-11 rounded-full bg-primary px-4 text-sm font-semibold text-white">刷新行情</button></form></div><FinanceNav /><MarketSwitch active="a" />
    ...
    {latestArtifact&&researchReport?<MarketResearchReport artifact={latestArtifact} content={researchReport} context={researchState.context} run={latestRun}/>:latestArtifact?<div className="mt-4 rounded-3xl bg-red-50 p-5 text-sm text-red-700">最近报告不是 Market Daily Brief v2，或未通过核心事实校验；市场事实未受影响。</div>:null}
  </main>;
}
