...
export default async function AnalysisPage({searchParams}:{searchParams:Promise<{error?:string;generated?:string;reused?:string}>}){
  ...
  const latest=artifacts[0]??null;const run=latest?.run_id?await data.aiArtifacts.getRun(user.id,latest.run_id):null;const active=providerStatus().find(item=>item.active);
