...
export async function getRun(client:SupabaseClient,userId:string,runId:string){const {data,error}=await client.from("ai_runs").select("id,provider,model,status,input_hash,input_tokens,output_tokens,total_tokens,latency_ms,error_message,started_at,completed_at").eq("user_id",userId).eq("id",runId).maybeSingle();if(error)throwDataAccessError("aiRuns.get",error);return data;}
