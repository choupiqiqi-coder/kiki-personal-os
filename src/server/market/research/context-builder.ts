...
import { buildFundRelationshipFacts,describeFundRelationship } from "@/lib/finance/fund-relationships";
...
async function collectContext(userId:string,previous:MarketResearchContext|null):Promise<MarketResearchContext>{
  ...
  const relationshipFacts=buildFundRelationshipFacts(funds);
  const exposure=relationshipFacts.map((fund,index)=>{const relationship=describeFundRelationship(fund);const factId=`fund-relation-${index+1}-${fund.fundCode}`;facts.push({factId,kind:"fund",statement:`${fund.fundName}：${relationship}；正式 NAV 日期 ${fund.navDate??"未知"}`,sourceId:"fund-holdings",sourceTimestamp:fund.navDate});return{...fund,factId,relationship,sourceId:"fund-holdings"};});
  ...
  const limitations=[...structure.limitations,...events.limitations,...news.limitations,...relationshipFacts.flatMap(fund=>fund.limitations)];if(!china.data)limitations.push("A 股核心指数快照不可用");if(!us.data)limitations.push("美股核心指数快照不可用");if(structure.style.length===0)limitations.push("V1 暂无独立小盘指数样本，市场风格只能结合核心指数与行业结构谨慎解释");if(exposure.some(x=>x.relationshipStatus==="unknown"))limitations.push("部分基金关系状态为 unknown，禁止根据名称猜测暴露");
