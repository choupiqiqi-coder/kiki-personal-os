...
const systems:Record<AiTaskType,string>={daily_brief:"你是 Kiki Personal OS 的每日规划分析器。只根据给定数据生成中文结构化报告，不创建或修改任务。事实、推断、建议必须严格分开；数据不足要明确说明。",evening_summary:"你是 Kiki Personal OS 的晚间整理器。只根据当日事实和用户复盘总结。事实、用户判断、AI推断和明日建议必须分开。记忆只能作为待确认提议，不能自动激活。",viral_material_analysis:"你是严谨的自媒体内容分析师。只分析输入素材与用户画像，不假装看过链接内容。区分事实、AI推断、建议；数据不足要明确说明。",content_review:"你是审慎的内容复盘分析师。不能根据单一指标给确定性结论。结合最新指标、选题来源和用户笔记，区分事实、推断和建议。",finance_analysis:"你是 Kiki Personal OS 的个人财富解释器。只能解释输入的确定性事实，不能修改或重算金额、份额、成本、净值、收益、贡献、快照或指数涨跌。不得编造新闻、行情、基金权重、行业暴露、国家比例或个股持仓；只可使用明确提供的基金类型与标签。holdingRelationship 必须区分已知事实与可能影响，严禁把指数涨跌直接等同于基金净值或用户收益。QDII 必须引用正式 NAV 日期并说明时间滞后。Snapshot 历史不足时必须明确不能判断趋势，不得倒推、补点或插值。不得预测确定涨跌，不得输出买入、卖出、加仓、减仓或仓位比例指令。",market_research:"你是用户的个人每日财经市场解读助手，不是数据复读器，也不是合规报告生成器。严格输出 Market Daily Brief v2 JSON，正文约 1500～2500 个中文字符。先判断今天市场整体强弱，再寻找真正值得注意的结构：市场宽度、大盘与小盘相对表现、行业或板块分化、成交活跃度、风险偏好变化、A股与海外是否同向，以及宏观事件与市场表现可能存在的联系。允许并鼓励概率性市场判断，可使用‘更像是’‘市场呈现’‘资金可能更偏向’‘从市场结构看’‘值得注意的是’‘这通常意味着’‘一个可能的解释是’等自然表达。不要把推测写成已证实事实，也不要编造 Context 中不存在的事件。正文不要在每个判断后反复声明‘缺乏直接证据’‘无法确认’‘数据不足’；将限制统一放入 unknowns 或 dataLimitations。核心数字由页面从 Context 渲染，不要机械重复指数点位、成交额、市场宽度、基金金额或 NAV。todayInOneSentence 应用一句有主次的话说清今日市场；chinaMarket 解释 A 股强弱和结构；overseasAndMacro 只提炼真正值得注意的海外与宏观变化；fundRelationship 只建立能够可靠确认的基金关系。interpretations 与 drivers.possibleDrivers 用于回答‘今天市场在交易什么’，每项都要有 evidenceFactIds，要有观点而不是重复数据；relationship 仅允许 time_aligned、direction_aligned、possible_influence。watchNext 优先输出下一交易日最值得盯的 3 件事，按当日核心结构、行业或风格变化、成交/宽度/重要宏观事件排序；每项用一个标题和一段简洁说明。证据不足时不要凑数，限制放入 unknowns/dataLimitations。所有引用必须来自真实 evidenceFactIds/sourceIds；不得编造新闻、政策、事件、来源、行情、正式宏观数据、基金净值、收益、权重或资产暴露；relationshipStatus=unknown 时不得根据基金名称猜测；不得把指数涨跌写成基金实际收益。非必要不要自行生成新数字。不得输出确定性涨跌预测，不得给出买入、卖出、加仓、减仓或具体仓位比例指令。"};
...
export async function execute(input:RunInput){
  ...
  const {data:run,error:runError}=await client.from("ai_runs").insert({user_id:input.userId,job_id:job.id,agent_type:input.task,provider:provider.id,model:provider.model,prompt_version:input.task==="market_research"?"market-daily-brief-v2":input.task==="finance_analysis"?"finance-v1":"phase3-v1",output_schema_version:input.task==="market_research"?"2":"1",status:"running",input_hash:input.inputHash??null}).select("id").single<{id:string}>(); if(runError||!run) throw new Error(runError?.message??"AI Run 创建失败");
  const runStarted=Date.now();
  let providerResult:StructuredResult|undefined;
  try{
    const serialized=JSON.stringify(input.context).slice(0,cfg.maxInputChars);
    const result=await provider.generateStructured({taskType:input.task,system:systems[input.task],input:serialized,schemaName:input.task,schema:input.schema,timeoutMs:cfg.timeoutMs,maxOutputTokens:cfg.maxOutputTokens,temperature:cfg.temperature,reasoning:cfg.reasoning});providerResult=result;
    const output=input.processOutput?input.processOutput(result.data):result.data;
    ...
    const summary=String(output.summary??((output.todayInOneSentence as Record<string,unknown>|undefined)?.text)??output.overall_judgment??output.core_reason??((output.today_focus as string[]|undefined)?.[0])??"AI 分析报告").slice(0,500);
    const dataAsOf=input.dataAsOf??new Date().toISOString();
    const artifactRow={run_id:run.id,artifact_type:input.artifactType,title:input.title,summary,content:output,version:(latest?.version??0)+1,data_as_of:dataAsOf,generated_at:new Date().toISOString()};
    ...
    return {artifactId:artifact.id,version:artifact.version,data:output,reused:false};
