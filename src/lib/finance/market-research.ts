...
function assertFieldNumbersSupported(field:string,text:string,context:MarketResearchContext){const sources=collectContextNumbers(context);for(const output of parseNumericMentions(text))if(!sources.some(source=>isDeterministicEquivalent(output,source)))reject(`${field} 包含无法追溯的基金事实数字: ${output.raw}`);}
function findNonCoreUngroundedNumber(text:string,context:MarketResearchContext){const sources=collectContextNumbers(context);for(const sentence of sentences(text))for(const output of parseNumericMentions(sentence))if(!sources.some(source=>isDeterministicEquivalent(output,source)))return`包含未登记的辅助数字 ${output.raw}`;return null;}
