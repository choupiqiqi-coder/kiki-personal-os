export type FinanceAnalysisOutput = { summary:string; marketReview:string; holdingRelationship:string; portfolioReview:string; watchNext:string[]; risks:string[]; dataLimitations:string[]; dataAsOf:string };
const keys=["summary","marketReview","holdingRelationship","portfolioReview","watchNext","risks","dataLimitations","dataAsOf"] as const;
export function assertFinanceAnalysisOutput(value:Record<string,unknown>):asserts value is FinanceAnalysisOutput {
  ...
  for(const key of ["summary","marketReview","holdingRelationship","portfolioReview","dataAsOf"] as const)if(typeof value[key]!=="string"||!value[key])throw new Error(`AI 分析字段 ${key} 无效`);
